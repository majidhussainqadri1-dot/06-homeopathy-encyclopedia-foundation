...
$g = file_get_contents( $plugin . '/includes/class-he-v24-future-review-guard.php' );
...
foreach ( array( 'class-he-v24-future-schema.php','class-he-v24-migration-safety.php','class-he-v24-future-api.php','class-he-v24-future-privacy.php','class-he-v24-future-review-guard.php' ) as $file ) { v24_has( $b, $file, 'bootstrap ' . $file ); }
v24_has( $b, 'future_v24_ready', 'v2.4 hooks gated on verified schema upgrade' );
...
v24_has( $g, 'he_future_claim_version_gate', 'strict current-version claim gate' );
v24_has( $g, "status='reviewed' AND review_required=0", 'human-reviewed external evidence required before claim approval' );
...
v24_has( $g, 'sanitize_provenance_response', 'provenance public DTO sanitizer' );
v24_has( $g, 'strip_internal_ids', 'provenance metadata internal-ID stripping' );
...
v24_has( $g, 'he_future_orcid_scope', 'generic evidence route rejects ORCID' );
v24_has( $g, 'he_future_mapping_scope', 'concept mapping route restricted to MeSH' );
v24_has( $g, '/future/external/(?P<id>\\d+)/review', 'explicit external scholarly human-review route' );
...
foreach ( array( '/future/public/claims/','/future/public/graph/','/future/public/time-machine/','/future/public/freshness/','/future/public/citations/','/future/public/translations/' ) as $route ) { v24_has( $g, $route, 'canonical UUID public route ' . $route ); }
...
v24_has( $g, "translation_version=%d AND status='draft'", 'atomic translation review state predicate' );
v24_has( $g, "translation_version=%d AND status='approved'", 'atomic translation publish state predicate' );
v24_has( $g, 'independent approval review', 'translation independent review' );
...
foreach ( array( 'HE_V2_Schema::OPTION_SAFE_MODE','HE_V2_Auth::require_nonce','HE_V2_Domain::rate_allow','Idempotency-Key','idempotent_begin','idempotent_finish' ) as $token ) { v24_has( $a . $g, $token, 'future mutation guard ' . $token ); }
