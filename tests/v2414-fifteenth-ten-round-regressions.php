...
$env=getenv('V2414_ROUND');$upto=(false!==$env&&''!==$env)?(int)$env:(preg_match('/ \* Version: 2\.4\.(?:14|15|16)/',$bootstrap)?10:0);
...
v2414_ok(10,preg_match('/ \* Version: 2\.4\.(?:14|15|16)/',$bootstrap)&&preg_match("/HE_VERSION', '2\\.4\\.(?:14|15|16)/",$bootstrap)&&preg_match("/HE_CONTRACT_VERSION', '2\\.4\\.(?:14|15|16)/",$bootstrap)&&false!==strpos($bootstrap,"'future_hardening_version'=>"),'historical v2.4.14 release controls do not tolerate the later current v2.4.x candidate');
v2414_ok(10,preg_match('/Stable tag: 2\.4\.(?:14|15|16)/',$readme)&&false!==strpos($runall,'v2414-fifteenth-ten-round-regressions.php')&&preg_match('/file06-v2\.4\.(?:14|15|16)-a\.zip/',$runall)&&preg_match('/All File 06 v2\.4\.(?:14|15|16) automated checks/',$runall),'historical aggregate/package controls no longer tolerate the current v2.4.x candidate');
