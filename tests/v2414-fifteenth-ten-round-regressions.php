...
$env=getenv('V2414_ROUND');$upto=(false!==$env&&''!==$env)?(int)$env:(preg_match('/ \* Version: 2\.4\.(?:14|15)/',$bootstrap)?10:0);
...
v2414_ok(3,false!==strpos($domain,'entry_create_compensation_start_failed')&&false!==strpos($domain,'entry-child-delete-failed-')&&false!==strpos($domain,'entry-create-compensation-commit-failed')&&false!==strpos($domain,'entry_create_post_compensation_failed'),'entry-create compensation can silently partially fail');
...
v2414_ok(9,false!==strpos($domain,"'retracted' !== \$row['status'] ? json_decode( \$row['case_json'], true ) : null")&&false!==strpos($domain,"'public' !== \$row['data_class'] || 'retracted' === \$row['status']")&&false!==strpos($domain,"'publish' !== \$post->post_status")&&false!==strpos($browse,"'public' === \$row['data_class'] && 'retracted' !== \$row['status']"),'retracted research REST/browse payloads or stale deleted posts can remain publicly detailed');
...
v2414_ok(10,preg_match('/ \* Version: 2\.4\.(?:14|15)/',$bootstrap)&&preg_match("/HE_VERSION', '2\\.4\\.(?:14|15)/",$bootstrap)&&preg_match("/HE_CONTRACT_VERSION', '2\\.4\\.(?:14|15)/",$bootstrap)&&false!==strpos($bootstrap,"'future_hardening_version'=>"),'historical v2.4.14 release controls do not tolerate the later current v2.4.x candidate');
v2414_ok(10,preg_match('/Stable tag: 2\.4\.(?:14|15)/',$readme)&&false!==strpos($runall,'v2414-fifteenth-ten-round-regressions.php')&&preg_match('/file06-v2\.4\.(?:14|15)-a\.zip/',$runall)&&preg_match('/All File 06 v2\.4\.(?:14|15) automated checks/',$runall),'historical aggregate/package controls no longer tolerate the current v2.4.x candidate');
