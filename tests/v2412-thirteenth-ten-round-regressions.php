...
v2412_ok(7,false!==strpos($domain,'public static function sanitize_text_list')&&false!==strpos($domain,"isset( \$value['statement'] )")&&substr_count($domain,'self::sanitize_text_list(')>=2&&false!==strpos($authoring,'HE_V2_Domain::sanitize_text_list( $value )')&&substr_count($third,'HE_V2_Domain::sanitize_text_list')>=2,'nested investigator/conflict shapes can trigger array-to-string warnings or inconsistent governance records');
...
v2412_ok(10,preg_match('/ \* Version: 2\.4\.(?:12|13|14|15)/',$bootstrap)&&preg_match("/HE_VERSION', '2\.4\.(?:12|13|14|15)/",$bootstrap)&&preg_match("/HE_CONTRACT_VERSION', '2\.4\.(?:12|13|14|15)/",$bootstrap)&&false!==strpos($bootstrap,"'future_hardening_version'=>"),'historical v2.4.12 release controls do not tolerate a later current v2.4.x candidate');
