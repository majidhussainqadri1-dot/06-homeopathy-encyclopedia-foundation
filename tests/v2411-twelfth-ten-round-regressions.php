...
v2411_ok(1,false!==strpos($guard,'public static function public_surface_eligible')&&false!==strpos($guard,"'dataset' === $row['record_type']")&&false!==strpos($guard,"array( 'restricted','highly-restricted' )")&&false!==strpos($gov,'HE_V22_Research_Guard::public_surface_eligible( $row )'),'dataset metadata permanent-ID route still conflicts with private-by-default dataset law');
...
v2411_ok(8,false!==strpos($guard,'$out[] = $item;')&&false!==strpos($authoring,"if ( is_array( $item ) ) { $item = $item['name'] ?? ''; }")&&false!==strpos($authoring,'$investigators = self::investigators('),'legacy nested investigator JSON can cause Array-to-string runtime warnings');
...
v2411_ok(10,false!==strpos($bootstrap,' * Version: 2.4.11')&&false!==strpos($bootstrap,"define( 'HE_VERSION', '2.4.11' );")&&false!==strpos($bootstrap,"define( 'HE_CONTRACT_VERSION', '2.4.11' );")&&false!==strpos($bootstrap,"'future_hardening_version'=>'2.4.11'"),'runtime/contract/future hardening truth is not v2.4.11');
