...
final class HE_V241_Research_Governance {
	...
	private static function finish( $reservation, $result, $code=200 ) {
		...
		if(is_wp_error($result)){ $d=$result->get_error_data();$s=is_array($d)&&isset($d['status'])?(int)$d['status']:400;HE_V2_Domain::idempotent_finish($reservation['id'],$s,array('code'=>$result->get_error_code(),'message'=>$result->get_error_message()));return $result; }
		HE_V2_Domain::idempotent_finish($reservation['id'],$code,$result);return new WP_REST_Response($result,$code);
	}
