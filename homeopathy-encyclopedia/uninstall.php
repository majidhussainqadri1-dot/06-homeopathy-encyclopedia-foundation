...
/**
 * A global/institutional legal or research-integrity hold may veto destructive
 * uninstall. Privacy implementations use the same hook with a concrete user ID;
 * user_id=0 means the entire File 06 corpus is being considered for deletion.
 */
if ( (bool) apply_filters( 'he_v2_privacy_legal_hold', false, 0 ) ) {
	return;
}
...
function he_v2_purge_site() {
	...
	// Core + v2.2 + Future-18/v2.4 tables. No companion-owned table is touched.
	foreach ( array(
	...
	foreach ( array(
		...
		'he_v23_future_version','he_v24_audit80_version','he_v24_knowledge_cursor'
	) as $option ) {
